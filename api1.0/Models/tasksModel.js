...
module.exports = {
    createTask: async (res, userId, context) => {
        ...
        try {
            ...
            const [result] = await connection.execute(query, [context.title, context.content, contextcreated_at, context.deadline, context.task_vacancy, context.location, context.reward, "pending", userId]) ;
            const response = {
                data: {
                    task: {
                        id: result.insertId
